...
public class level1m_diag {
    public static void main(String[] args)
    {
        ...
        blis.bli_printm( "a: randomize upper part (lower part may contain garbage)", a, "% 4.3f", "" );
        ...
        blis.bli_obj_set_struc( BLIS_TRIANGULAR, b );
        blis.bli_obj_set_uplo( BLIS_UPPER, b );
        ...
        // object (ie: objects created via blis.bli_obj_create*()) defaults to 0,
        ...
        blis.bli_obj_set_diag_offset( -1, bl );

        // Set the uplo field of 'bl' to "lower".
        blis.bli_obj_set_uplo( BLIS_LOWER, bl );
        ...
        blis.bli_setm( BLIS_ZERO.toptr(), bl );

        blis.bli_printm( "b: randomize upper part; set strictly lower part to 0.0", b, "% 4.3f", "" );
        ...
        blis.bli_setm( BLIS_MINUS_ONE.toptr(), bl );

        blis.bli_printm( "b: randomize upper part; set strictly lower part to -1.0", b, "% 4.3f", "" );
        ...
        blis.bli_obj_set_diag_offset( 0, bl );
        ...
        blis.bli_printm( "c: copy lower part of b (upper part may contain garbage)", c, "% 4.3f", "" );
        ...
        blis.bli_printm( "a: copy lower triangular bl to upper triangular a", a, "% 4.3f", "" );
        ...
        blis.bli_setm( BLIS_ZERO.toptr(), d );

        blis.bli_printm( "d: initial value (all zeros)", d, "% 4.3f", "" );
        ...
        blis.bli_setijm( 2.0, 0.0, 2, 0, bl );
        blis.bli_setijm( 3.0, 0.0, 3, 0, bl );
        blis.bli_setijm( 4.0, 0.0, 4, 0, bl );
        blis.bli_setijm( 3.1, 0.0, 3, 1, bl );
        blis.bli_setijm( 3.2, 0.0, 3, 2, bl );

        blis.bli_printm( "bl: lower triangular bl is aliased to b", bl, "% 4.3f", "" );

        // We want to pluck out the lower triangle and transpose it into the upper
        // triangle of 'd'.
        blis.bli_obj_toggle_trans( bl );
        ...
        blis.bli_printm( "d: transpose of lower triangular of bl copied to d", d, "% 4.3f", "" );
        ...
        blis.bli_printm( "e: initial value (all -1.0)", e, "% 4.3f", "" );

        // Create an alias to work with.
        blis.bli_obj_alias_to( e, el );

        // Set structure and uplo of 'el'.
        blis.bli_obj_set_struc( BLIS_TRIANGULAR, el );
        blis.bli_obj_set_uplo( BLIS_LOWER, el );
        ...
        blis.bli_printm( "e: after lower trapezoid randomized", e, "% 4.3f", "" );

        // Move the diagonal offset of 'el' to 1 and flip the uplo field to
        // "upper".
        blis.bli_obj_set_diag_offset( 1, el );
        blis.bli_obj_set_uplo( BLIS_UPPER, el );

        // Set the upper triangle to zero.
        blis.bli_setm( BLIS_ZERO.toptr(), el );

        blis.bli_printm( "e: after upper triangle set to zero", e, "% 4.3f", "" );
        ...
        dt = BLIS_DOUBLE;
        m = 5; n = 5; rs = 0; cs = 0;
        blis.bli_obj_create( dt, m, n, rs, cs, h );

        // Initialize the entire matrix to -1.0 to simulate junk values.
        blis.bli_setm( BLIS_MINUS_ONE.toptr(), h );

        blis.bli_printm( "h: initial value (all -1.0)", h, "% 4.3f", "" );

        // Set the diagonal offset of 'h' to -1.
        blis.bli_obj_set_diag_offset( -1, h );

        // Set the structure and uplo of 'h'.
        blis.bli_obj_set_struc( BLIS_TRIANGULAR, h );
        blis.bli_obj_set_uplo( BLIS_UPPER, h );

        // Randomize the elements on and above the first subdiagonal.
        blis.bli_randm( h );

        blis.bli_printm( "h: after randomizing above first subdiagonal", h, "% 4.3f", "" );

        // Create an alias to work with.
        blis.bli_obj_alias_to( h, hl );

        // Flip the uplo of 'hl' and move the diagonal down by one.
        blis.bli_obj_set_uplo( BLIS_LOWER, hl );
        blis.bli_obj_set_diag_offset( -2, hl );

        // Set the region strictly below the first subdiagonal (on or below
        // the second subdiagonal) to zero.
        blis.bli_setm( BLIS_ZERO.toptr(), hl );

        blis.bli_printm( "h: after setting elements below first subdiagonal to zero", h, "% 4.3f", "" );
