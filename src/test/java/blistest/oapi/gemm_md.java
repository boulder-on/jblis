package blistest.oapi;

import jblis.blisFactory;
import jblis.types.num_t;
import jblis.types.obj_t;

import static jblis.oapi.blis_oapi.BLIS_ONE;
import static jblis.oapi.blis_oapi.BLIS_ZERO;
import static jblis.types.num_t.*;
import static jblis.types.prec_t.BLIS_DOUBLE_PREC;
import static jblis.types.prec_t.BLIS_SINGLE_PREC;

public class gemm_md {
    public static void main(String[] args)
    {
        num_t dt_r, dt_c;
        num_t dt_s, dt_d;
        num_t dt_a, dt_b;
        long m, n, k;
        long rs, cs;

        obj_t[] a = obj_t.ptr(),
                b = obj_t.ptr(),
                c = obj_t.ptr();
        obj_t[] alpha = obj_t.ptr();
        obj_t[] beta = obj_t.ptr();

        var blis = blisFactory.getBlisOAPI();

        //
        // This file demonstrates mixing datatypes in gemm.
        //
        // NOTE: Please make sure that mixed datatype support is enabled in BLIS
        // before proceeding to build and run the example binaries. If you're not
        // sure whether mixed datatype support is enabled in BLIS, please refer
        // to './configure --help' for the relevant options.
        //

        //
        // Example 1: Perform a general matrix-matrix multiply (gemm) operation
        //            with operands of different domains (but identical precisions).
        //

        System.out.print( "\n#\n#  -- Example 1 --\n#\n\n" );

        // Create some matrix operands to work with.
        dt_r = BLIS_DOUBLE;
        dt_c = BLIS_DCOMPLEX;
        m = 4; n = 5; k = 1; rs = 0; cs = 0;
        blis.bli_obj_create( dt_c, m, n, rs, cs, c );
        blis.bli_obj_create( dt_r, m, k, rs, cs, a );
        blis.bli_obj_create( dt_c, k, n, rs, cs, b );

        // Set the scalars to use.
        alpha = BLIS_ONE.toptr();
        beta  = BLIS_ONE.toptr();

        // Initialize the matrix operands.
        blis.bli_randm( a );
        blis.bli_randm( b );
        blis.bli_setm( BLIS_ZERO.toptr(), c );

        blis.bli_printm( "a (double real):    randomized", a, "% 4.3f", "" );
        blis.bli_printm( "b (double complex): randomized", b, "% 4.3f", "" );
        blis.bli_printm( "c (double complex): initial value", c, "% 4.3f", "" );

        // c := beta * c + alpha * a * b, where 'a' is real, and 'b' and 'c' are
        // complex.
        blis.bli_gemm( alpha, a, b, beta, c );

        blis.bli_printm( "c (double complex): after gemm", c, "% 4.3f", "" );

        // Free the objects.
        blis.bli_obj_free( a );
        blis.bli_obj_free( b );
        blis.bli_obj_free( c );

        //
        // Example 2: Perform a general matrix-matrix multiply (gemm) operation
        //            with operands of different precisions (but identical domains).
        //

        System.out.print( "\n#\n#  -- Example 2 --\n#\n\n" );

        // Create some matrix operands to work with.
        dt_s = BLIS_FLOAT;
        dt_d = BLIS_DOUBLE;
        m = 4; n = 5; k = 1; rs = 0; cs = 0;
        blis.bli_obj_create( dt_d, m, n, rs, cs, c );
        blis.bli_obj_create( dt_s, m, k, rs, cs, a );
        blis.bli_obj_create( dt_s, k, n, rs, cs, b );

        // Notice that we've chosen C to be double-precision real and A and B to be
        // single-precision real.

        // Since we are mixing precisions, we will also need to specify the
        // so-called "computation precision." That is, we need to signal to
        // blis.bli_gemm() whether we want the A*B product to be computed in single
        // precision or double precision (prior to the result being accumulated
        // back to C). To specify the computation precision, we need to set the
        // corresponding bit in the C object. Here, we specify double-precision
        // computation.
        // NOTE: If you do not explicitly specify the computation precision, it
        // will default to the storage precision of the C object.
        blis.bli_obj_set_comp_prec( BLIS_DOUBLE_PREC, c );

        // Initialize the matrix operands.
        blis.bli_randm( a );
        blis.bli_randm( b );
        blis.bli_setm( BLIS_ZERO.toptr(), c );

        blis.bli_printm( "a (single real): randomized", a, "% 4.3f", "" );
        blis.bli_printm( "b (single real): randomized", b, "% 4.3f", "" );
        blis.bli_printm( "c (double real): initial value", c, "% 4.3f", "" );

        // c := beta * c + alpha * a * b, where 'a' and 'b' are single-precision
        // real, 'c' is double-precision real, and the matrix product is performed
        // in double-precision arithmetic.
        blis.bli_gemm( alpha, a, b, beta, c );

        blis.bli_printm( "c (double real): after gemm (exec prec = double precision)", c, "% 4.3f", "" );

        // Free the objects.
        blis.bli_obj_free( a );
        blis.bli_obj_free( b );
        blis.bli_obj_free( c );

        //
        // Example 3: Perform a general matrix-matrix multiply (gemm) operation
        //            with operands of different domains AND precisions.
        //

        System.out.print( "\n#\n#  -- Example 3 --\n#\n\n" );

        // Create some matrix operands to work with.
        dt_a = BLIS_FLOAT;
        dt_b = BLIS_DCOMPLEX;
        dt_c = BLIS_SCOMPLEX;
        m = 4; n = 5; k = 1; rs = 0; cs = 0;
        blis.bli_obj_create( dt_c, m, n, rs, cs, c );
        blis.bli_obj_create( dt_a, m, k, rs, cs, a );
        blis.bli_obj_create( dt_b, k, n, rs, cs, b );

        // Notice that we've chosen C to be single-precision complex, and A to be
        // single-precision real, and B to be double-precision complex.

        // Set the computation precision to single precision this time.
        blis.bli_obj_set_comp_prec( BLIS_SINGLE_PREC, c );

        // Initialize the matrix operands.
        blis.bli_randm( a );
        blis.bli_randm( b );
        blis.bli_setm( BLIS_ZERO.toptr(), c );

        blis.bli_printm( "a (single real): randomized", a, "% 4.3f", "" );
        blis.bli_printm( "b (double complex): randomized", b, "% 4.3f", "" );
        blis.bli_printm( "c (single complex): initial value", c, "% 4.3f", "" );

        // c := beta * c + alpha * a * b, where 'a' is single-precision real, 'b'
        // is double-precision complex, 'c' is single-precision complex, and the
        // matrix product is performed in single-precision arithmetic.
        blis.bli_gemm( alpha, a, b, beta, c );

        blis.bli_printm( "c (single complex): after gemm (exec prec = single precision)", c, "% 4.3f", "" );

        // Free the objects.
        blis.bli_obj_free( a );
        blis.bli_obj_free( b );
        blis.bli_obj_free( c );

        //
        // Example 4: Project objects between the real and complex domains.
        //

        System.out.print( "\n#\n#  -- Example 4 --\n#\n\n" );

        // Create some matrix operands to work with.
        dt_r = BLIS_DOUBLE;
        dt_c = BLIS_DCOMPLEX;
        m = 4; n = 5; rs = 0; cs = 0;
        blis.bli_obj_create( dt_r, m, n, rs, cs, a );
        blis.bli_obj_create( dt_c, m, n, rs, cs, b );

        // Initialize a real matrix A.
        blis.bli_randm( a );

        blis.bli_printm( "a (double real): randomized", a, "% 4.3f", "" );

        // Project real matrix A to the complex domain (in B).
        blis.bli_projm( a, b );

        blis.bli_printm( "b (double complex): projected from 'a'", b, "% 4.3f", "" );

        // Notice how the imaginary components in B are zero since any real
        // matrix implicitly has imaginary values that are equal to zero.

        // Now let's project in the other direction.

        // Initialize the complex matrix B.
        blis.bli_randm( b );

        blis.bli_printm( "b (double complex): randomized", b, "% 4.3f", "" );

        // Project complex matrix B to the real domain (in A).
        blis.bli_projm( b, a );

        blis.bli_printm( "a (double real): projected from 'b'", a, "% 4.3f", "" );

        // Notice how the imaginary components are lost in the projection from
        // the complex domain to the real domain.

        // Free the objects.
        blis.bli_obj_free( a );
        blis.bli_obj_free( b );

        //
        // Example 5: Typecast objects between the single and double precisions.
        //

        System.out.print( "\n#\n#  -- Example 5 --\n#\n\n" );

        // Create some matrix operands to work with.
        dt_s = BLIS_FLOAT;
        dt_d = BLIS_DOUBLE;
        m = 4; n = 3; rs = 0; cs = 0;
        blis.bli_obj_create( dt_d, m, n, rs, cs, a );
        blis.bli_obj_create( dt_s, m, n, rs, cs, b );

        // Initialize a double-precision real matrix A.
        blis.bli_randm( a );

        blis.bli_printm( "a (double real): randomized", a, "%23.16e", "" );

        // Typecast A to single precision.
        blis.bli_castm( a, b );

        blis.bli_printm( "b (single real): typecast from 'a'", b, "%23.16e", "" );

        // Notice how the values in B are only accurate to the 6th or 7th decimal
        // place relative to the true values in A.

        // Free the objects.
        blis.bli_obj_free( a );
        blis.bli_obj_free( b );

    }
}
